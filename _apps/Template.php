			...
			if( $this->data[auth_session_key] ){
				...
				if( isset($this->data['auth_time']) ){				
					...
					if( $s_dif <= ($this->session_timout * 60) ){
						...
						if($expired){
							require_once "./_store/authconfig.php";
							$Hybridauth = new Hybridauth\Hybridauth($authconfig);	
							//Check and disconnect all connected adapters
							$adapters = $Hybridauth->getConnectedAdapters();
							$countadapters = count($adapters);
							if($countadapters){
								foreach($adapters as $conad){
									$conad->disconnect();
								}
							}
							$this->expire();
				...
				//Not logged in//
				$Template->data["page_destination"] = $_SERVER['HTTP_REFERER'];
				$Template->save();
